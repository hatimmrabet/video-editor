...
Writes: <work>/build/transcript-raw.json  = {"text":..., "segments":[{id,start,end,text,words:[{word,start,end}]}], "language":..., "model":...}
Also always prints "model: <name>" — which model actually ran is never left to guesswork.
...
def model_label(model):
    """A human-readable name for the log line — never just a bare local path."""
    if model == _DARIJA_CT2:
        return "darija fine-tune (anaszil/whisper-large-v3-turbo-darija)"
    return model
...
def to_a_json(segments, language, out_path, model_used=""):
    segs, full = [], []
    ...
    json.dump({"text": " ".join(full), "segments": segs, "language": language, "model": model_used},
...
def run_faster_whisper(wav, language, model, device, hard):
    ...
    print(f"🚀 faster-whisper · {model_label(model)} · {device} · {compute}")
...
def run_openai_whisper(wav, language, model, hard):
    import whisper
    print(f"🐢 openai-whisper · {model_label(model)} · cpu")
...
def main():
    ...
    print(f"model: {label}")   # always visible — never silently unclear which one ran
    if hard:
        print(f"⚠️  Hard dialect — the automatic transcription is still likely wrong: "
              "Claude re-reads and corrects the whole transcript before captioning (SKILL.md step 5).")
        if model == "large-v3":
            print("   (no darija fine-tune found — run: uv run scripts/prepare_darija_model.py)")
    ...
    try:
        ...
        if engine == "faster-whisper" and have("whisper"):
            ...
            used_label = model_label(fallback_model)
            print(f"⚠️  faster-whisper failed ({e}) — trying openai-whisper ({used_label})…")
            segs, detected = run_openai_whisper(wav, language, fallback_model, hard)
        else:
            raise

    to_a_json(segs, detected, out, model_used=used_label)
