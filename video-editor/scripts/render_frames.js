/* Composites the video + motion graphics and writes frames.
   node render_frames.js <workdir> all                       ← resumes where it stopped (skips what's done)
   node render_frames.js <workdir> all --force               ← redraws everything from scratch
   node render_frames.js <workdir> range 12.0 18.5           ← redraws just one window (after editing a scene)
...
(async()=>{
  ...
  /* ⚠️ the font wait must come **after** init: the non-Cairo font is injected inside init
     ...
     600 was missing before and it fell back to a substitute font on its own. */
  ...
  if(!fontOK) console.log('⚠️ font '+FF+' did not finish loading — rendering continues with a fallback font');
  const grab=async(t,file,q)=>{
    ...
    if(BEHIND){                                   // the person-cutout image for this frame (if any)
  ...
  if(mode==='preview'){
    ...
    for(const t of process.argv.slice(4).map(Number)){await grab(t,W+'build/prev/t'+t.toFixed(2)+'.jpg',0.9);console.log('preview',t);}
  }else{
    fs.mkdirSync(W+'build/frames-composited',{recursive:true});
    const n=Math.round(dur*FPS);
    const force=process.argv.includes('--force');
    let i0=0,i1=n;                       // optional time window — always overwritten
    if(mode==='range'){
      const a=Number(process.argv[4]), b=Number(process.argv[5]);
      if(!isFinite(a)||!isFinite(b)) throw new Error('range needs two times: <from> <to>');
    ...
    for(let i=i0;i<i1;i++){
      ...
      if(drawn%150===1)console.log('frame',i,'/',i1);
    }
    if(skipped)console.log('skipped',skipped,'frame(s) already done (resume) — --force redoes all of it');
    console.log('done',drawn,'frame(s) drawn out of',n,'— duration',dur.toFixed(3));
    if(mode!=='range'){                  // a missing frame = a broken mux, must be caught now
      let miss=0; for(let i=0;i<n;i++) if(!done(W+'build/frames-composited/'+String(i).padStart(5,'0')+'.jpg')) miss++;
      if(miss)console.log('⚠️ missing',miss,'frame(s) — re-run before encode.sh');
    }
  }
  await b.close();
})();
