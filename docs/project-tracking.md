...
| chore: CI gate (`.github/workflows/ci.yml`) | `type:chore` | ✅ — one job, < 2 min: static checks (coverage + `node --check` / `compileall` / `bash -n` / JSON parse / Remotion lockfile) then the headless suite. On every PR to `develop` / `main`. |
